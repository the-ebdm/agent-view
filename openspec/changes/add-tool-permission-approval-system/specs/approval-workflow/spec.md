...
Defines the tool permission approval workflow that allows users to grant or deny tool access to agents at runtime when the agent attempts to use a tool outside its configured permissions.
...
- **agent-management**: Extends agent session management with approval tracking
- **ui-dashboard**: Adds approval indicators and drawer to existing agent cards and dashboard
- **streaming-output**: Approval requests should ideally appear in agent message stream (future enhancement)
