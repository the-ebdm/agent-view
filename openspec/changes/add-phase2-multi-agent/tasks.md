...
- [ ] 1.1 Add `AgentName` type (auto-generated + editable)
- [ ] 1.2 Add `ToolPermission` type and `ToolPermissionPreset` enum
- [ ] 1.3 Add `AgentLifecycleState` enum (running, paused, stopped, error)
- [ ] 1.4 Extend `AgentSession` with `name`, `toolPermissions`, `lifecycleState` fields
- [ ] 1.5 Add `AgentMetrics` type (elapsed time, message count, last activity)

## 2. Session Manager Refactor
- [ ] 2.1 Remove single-agent enforcement (delete `currentAgentId`, `terminateSession` on spawn)
- [ ] 2.2 Add `activeAgents: Map<string, AgentSession>` for concurrent tracking
- [ ] 2.3 Implement `pauseAgent(id: string)` method
- [ ] 2.4 Implement `resumeAgent(id: string)` method
- [ ] 2.5 Implement `stopAgent(id: string)` method (graceful shutdown)
- [ ] 2.6 Add `getAllActiveAgents()` method
- [ ] 2.7 Add `getAgentMetrics(id: string)` method
- [ ] 2.8 Update `createSession()` to accept `name` and `toolPermissions` parameters

## 3. SDK Wrapper Enhancements
- [ ] 3.1 Support multiple concurrent `query()` instances (one per agent)
...
- [ ] 3.5 Pass `toolPermissions` to SDK query options
...
- [ ] 4.1 Define tool permission presets (read-only, standard, full-access)
- [ ] 4.2 Create tool allowlist validation function
- [ ] 4.3 Map permission preset to SDK tool configuration
- [ ] 4.4 Store permissions in agent session metadata
- [ ] 4.5 Validate tool usage against permissions during streaming

## 5. API Routes - New Endpoints
- [ ] 5.1 Create `/api/agents` (GET) - list all active agents
- [ ] 5.2 Create `/api/agents/[id]/pause` (POST) - pause agent
- [ ] 5.3 Create `/api/agents/[id]/resume` (POST) - resume agent
- [ ] 5.4 Create `/api/agents/[id]/stop` (POST) - stop agent
- [ ] 5.5 Create `/api/agents/[id]/restart` (POST) - restart with same config
- [ ] 5.6 Create `/api/agents/[id]/rename` (PUT) - update agent name

## 6. API Routes - Modified Endpoints
- [ ] 6.1 Update `/api/agents/spawn` to accept `name` parameter (optional)
- [ ] 6.2 Update `/api/agents/spawn` to accept `toolPermissions` parameter
- [ ] 6.3 Update `/api/agents/[id]/status` to include lifecycle state and metrics
- [ ] 6.4 Update `/api/agents/[id]/stream` to support paused state (queue messages)

## 7. Agent Naming System
- [ ] 7.1 Create `generateAgentName()` utility (e.g., "Agent Bravo-7", "Agent Delta-3")
- [ ] 7.2 Validate user-provided names (max length, allowed characters)
- [ ] 7.3 Ensure name uniqueness across active agents
