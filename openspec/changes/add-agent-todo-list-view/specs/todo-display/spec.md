...
## Purpose
Enable users to monitor agent task progress through a structured todo list view that displays real-time task status, completion progress, and current activity from the Claude Agent SDK's TodoWrite tool.
...
None - this is a new capability with no removals from existing specs.
